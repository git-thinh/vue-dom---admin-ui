			...
			<o-menu>
				<o-menu-list label="Navigation">
					<o-menu-item icon="home fa-fw fa-mr">
						...
						<o-menu-item v-for="room in ordered" :key="room.id" :to="{name: 'room', params: {id: room.id}}"
							:label="room.name" :icon="'fa-fw fa-mr ' + (room.icon ? room.icon : 'fas fa-cube')"
							tag="router-link" />
					</o-menu-item>
					...
					<o-menu-item v-if="dataStore.activeUserViews.length" icon="binoculars fa-fw fa-mr">
						...
						<o-menu-item v-for="userView in dataStore.activeUserViews" :key="userView.code"
							:to="{name: 'view', params: {code: userView.code}}" :label="userView.title"
							:icon="'fa-fw fa-mr ' + userView.icon" tag="router-link" />
					</o-menu-item>
					...
					<o-menu-item v-if="dataStore.tagsList.length" icon="tags fa-fw fa-mr">
						...
						<o-menu-item v-for="tag in dataStore.tagsList" :key="tag" :to="{name: 'tag', params: {tag}}"
							:label="tag" tag="router-link" />
					</o-menu-item>
					...
					<o-menu-item icon="bell fa-fw fa-mr" tag="router-link" :to="{name: 'notifications'}">
						<template #label>
							Notifications
							<notifications-counter class-name="is-inline ml-2" :is-link="false" />
						</template>
					</o-menu-item>

					<o-menu-item v-if="authStore.hasRole('user')" icon="user fa-fw fa-mr" tag="router-link"
						:to="{name: 'profile'}">
						<template #label>
							Profile
						</template>
					...
					<o-menu-item v-if="authStore.hasRole('admin')" icon="tools fa-fw fa-mr">
						...
						<o-menu-item :to="{name: 'admin-nlp'}" label="Traitement automatique du langage naturel"
							tag="router-link" />
				...
				<o-menu-list label="Actions">
					...
					<o-menu-item :icon="isDarkMode ? 'toggle-on fa-fw fa-mr':'toggle-off fa-fw fa-mr'" tag="a"
						@click="setDarkMode">
						<template #label>
							Mode sombre
						</template>
					</o-menu-item>
					<o-menu-item :icon-pack="isHomepage ? 'fas' : 'far'"
						:icon="isHomepage ? 'ban fa-fw fa-mr':'bookmark fa-fw fa-mr'" tag="a" @click="setHomepage">
						<template #label>
							Page par défaut
						</template>
					</o-menu-item>
...
<script>
	...
	import {
		useRoomsTree
	} from '@/composables/useRoomsTree.js'
	import {
		useAppStore
	} from '@/store/app'
	import {
