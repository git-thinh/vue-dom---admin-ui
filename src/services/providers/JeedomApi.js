...
const JeedomApi = function(jsonRpcApiUrl = null, websocketUrl = null, readDelay = 5000, statisticsPeriod = 86400000,
	trendPeriod = 3600000, trendThreshold = 0.1) {
		...
		events.result.forEach((event) => {
			switch (event.name) {
				case 'cmd::update': {
					...
					const currentValue = (state && state.type === 'boolean') ? event.option.value === 1 :
						event.option.value
					updateCmds.push({
				...
				case 'jeeObject::summary::update':
					for (const key in event.option.keys) {
						dataStore.saveRoomSummary({
							id: event.option.object_id,
							key,
							value: event.option.keys[key].value
						})
					}
					break
	...
	return {
		...
		async authenticate(login, password) {
			try {
				return await jsonRpcCall('user::getHash', {
					login,
					password
				})
			} catch (error) {
				...
				switch (event.code) {
					...
					case 1006:
						...
						if (!appStore.hasNetwork) {
							// no network
							console.warn(
								`Network failure, events socket connection closed (code: ${event.code}, try #${socketErrorCount}/${socketMaxTry})`
								)
							return
						}
						if (socketErrorCount >= socketMaxTry) {
							console.warn(
								`Events socket connection closed (code: ${event.code}, try #${socketErrorCount}/${socketMaxTry})`
								)
							this.openEventsListenerFallback(true, appStore, dataStore)
							return
						}
						// try to reconnect
						console.warn(
							`Events socket connection closed (code: ${event.code}, try #${socketErrorCount}/${socketMaxTry}), reconnecting...`
							)
						this.openEventsListener(false, false)
						break
					default:
						console.warn(
							`Events socket connection closed (code: ${event.code}, try #${socketErrorCount})`
							)
				}
			}
		},
		...
		async openEventsListenerFallback(isPolling, appStore, dataStore) {
			try {
				appStore.setEventsListenerStatus(true)
				const events = await jsonRpcCall('event::changes', {
					datetime: lastEventsTimestamp
				})
				lastEventsTimestamp = events.datetime
		...
		async getRooms() {
			try {
				...
				return jObjects.map((jObject) => {
					...
					// set room equipments
					room.equipments = jObject.eqLogics.filter((jEqLogic) => jEqLogic.isVisible === '1')
						.map((jEqLogic) => {
							// construct equipment
							const equipment = {
								id: jEqLogic.id,
							...
							if (jEqLogic.status) {
								...
								if (jEqLogic.status.batterydanger || jEqLogic.status
									.batterywarning) {
									equipment.hasLowBattery = true
								}
								if (jEqLogic.status.lastCommunication) {
									equipment.lastCommunication = dtFormat(jEqLogic.status
										.lastCommunication, 'ISO')
								}
								if (jEqLogic.status.timeout || jEqLogic.status.warning || jEqLogic
									.status.danger) {
									equipment.hasNoCommunication = true
								}
							}
							// set equipment actions
							equipment.actions = jEqLogic.cmds.filter((jCmd) => jCmd.type ===
								'action').sort((a, b) => a.order - b.order).map((jCmd) => {
								const action = {
									id: jCmd.id,
								...
								if (jCmd.configuration.minValue && jCmd.configuration
									.minValue !== '') {
									action.minValue = parseInt(jCmd.configuration.minValue)
								}
								if (jCmd.configuration.maxValue && jCmd.configuration
									.maxValue !== '') {
									action.maxValue = parseInt(jCmd.configuration.maxValue)
								}
								if (jCmd.configuration.actionConfirm && jCmd.configuration
									.actionConfirm === '1') {
							...
							equipment.states = jEqLogic.cmds.filter((jCmd) => jCmd.type === 'info')
								.sort((a, b) => a.order - b.order).map((jCmd) => {
									const state = {
										id: jCmd.id,
										logicalId: jCmd.logicalId,
										name: jCmd.name,
		...
		async getRoomSummary(roomId, key) {
			try {
				return await jsonRpcCall('summary::byId', {
					id: roomId,
					key
				})
			} catch (error) {
		...
		async changeScenarioState(id, state) {
			try {
				return await jsonRpcCall('scenario::changeState', {
					id,
					state
				})
			} catch (error) {
